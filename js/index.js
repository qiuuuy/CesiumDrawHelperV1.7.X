...
$(function () {
    ...
    function initialGlobeView() {
        ...
        var image_Source = new Cesium.ArcGisMapServerImageryProvider({
...
//-------------------------------------------
        ...
        $("#exportBtn").click(function () {
            var blob = new Blob([JSON.stringify(shapeDic)], {type: "text/plain;charset=utf-8"});
            saveAs(blob, "场景数据.json");
        })
        //导入
        $("#insideBtn").click(function () {
            alert(`导入本地文件"http://127.0.0.1:5500/js/data.json"`);
            $.ajax({url:"./js/data.json",success:function(res){
                // let shapeDicIn = JSON.parse(strJson);
                let shapeDicIn = res[0].data;
                for (var key in shapeDicIn) {
                    if (shapeDicIn[key].radius) {
                        eval(`${shapeDicIn[key].backFun}(${key},${JSON.stringify(shapeDicIn[key].positions)},${shapeDicIn[key].radius})`);
                    } else {
                        eval(`${shapeDicIn[key].backFun}(${key},${JSON.stringify(shapeDicIn[key].positions)})`);
                    }
                }
                shapeDic = shapeDicIn;
            }});
...
//------------------------------------
    function editPolygon(objId) {
        var oldPositions = shapeDic[objId].positions;
    ...
    function editPoint(objId) {
        var oldPosition = shapeDic[objId].positions;
